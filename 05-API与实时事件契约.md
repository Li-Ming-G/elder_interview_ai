...
reconciliation 必须覆盖两个 durable crash 窗口：`question_generate` 已 freeze 为 pending/running 但 attempt 尚未创建，以及 attempt 已 running 但 complete/写回未终结。超过跨实例 grace 后可安全接管或将 orphan job/attempt 确定性终结为 failed/unavailable；不得因 replayed job 抛错、或因看到既有 attempt 直接 return 而永久卡住。进程启动、ACK 丢失与并发通知最终均须 terminal，且同 consumer 最多一个 committed attempt。
