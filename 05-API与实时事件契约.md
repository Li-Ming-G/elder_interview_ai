...
- `asr.interim` 使用稳定 `hypothesis_id` 和递增 `revision`，只替换临时显示且不落库；payload 必须包含服务端权威 `content_kind=conversation|speaker_calibration`，分类使用同 speaker stream 上标准化 `start_ms/end_ms` 与持久 attempt 半开区间的非零重叠，不使用客户端 revision 或 attempt 当前状态猜测；
...
- begin 只能在 formal start、正式 provider stream ready 且存在同 stream `speaker_stream_id` 后发生。临时麦克风检查流不得调用 begin 或创建 attempt。
- 普通 `GET /sessions/:id/transcripts` 只返回 `content_kind=conversation`；`speaker_calibration` 证据仍在数据库与校准/审计域保留，不通过普通实时转录或回顾正文返回。
- WebSocket calibration snapshot/event 保持原契约。`asr.interim` 在 WS 1.1 增加必填 `content_kind`；权威来源与 final 相同，按同 speaker stream 的持久 attempt 时间区间重叠判定，所以 resolve/skip 后迟到且 revision 更高的校准 interim 仍为 `speaker_calibration`。普通工作台只消费 `content_kind=conversation` 的 interim/final；不得按 revision 或 UI 时序猜测，也不得永久关闭后续 conversation interim。
- 降级继续时，如服务端 snapshot 已存在 attempt，客户端必须通过唯一 `resolveSpeakerCalibration(action=skip)` 边界冻结终点后才进入普通工作台。只有 provider unavailable 且 snapshot 明确不存在 attempt 时可以本地 degraded bypass；后续 provider 恢复不得在已显示的普通工作台后台自动开启隐藏 attempt。

### 10.3 自动收尾与本机 complete ACK
...
两个 basis lane 均 terminal 后才创建稳定 system request/trigger：`session_id + basis_analysis_trigger_identity + calibration_gate_identity`。calibration terminal attempt 必须属于 consumer 当前 active speaker stream 及其当前 active/preparing capture generation，identity 使用 `speaker_stream_id + attempt_id + terminal_status`；旧 stream terminal 不得满足重连后的新 stream。provider unavailable 且明确无 attempt时使用当前 `capture_generation_id + provider-unavailable policy revision`。同组合通过 request/trigger dedupe；同 consumer 另以 session consumed fence 保证 gate identity 变化后仍最多一个 `second_session_opening` attempt。analysis-first 由 calibration terminal 事件协调；calibration-first 由最后一个 lane terminal 事件协调；启动恢复由后台 reconciliation 读取持久事实重算。GET/list/WS replay 只读且不触发。

basis `succeeded` 的 eligible output 必须进入随后冻结的 Context；`unjudged|failed|cancelled|unavailable` 作为 terminal 如实记录，省略无法证明的本次新增 membership 后继续一次 opening，不等待无限 retry。snapshot 持久绑定 basis session/analysis trigger、calibration gate、两 lane outcome+job ID 与实际 membership；Director 只能以该 snapshot 作为 source 冻结完全相同的 scope/segment/memory/actual membership，不得二次抓取当前集合。某 lane 在 opening attempt 已提交后通过显式 retry 得到新 terminal outcome，也不得为同一 consumer session 重开 gate。failed/skipped/unavailable calibration gate 不得携带当前 stream trusted membership。结果继续通过既有 suggestion REST 与无正文 WS 1.2 投影；不新增正文 endpoint 或第二套 history。LLM unavailable 以既有 `kind=unavailable`/failed attempt 诚实返回，不自动回退 basic 题，不阻塞 recording。

reconciliation 必须覆盖三个 durable crash 窗口：`context_snapshot` job 已 freeze 但 `InterviewContextSnapshot` 尚未写回、`question_generate` 已 freeze 为 pending/running 但 attempt 尚未创建，以及 attempt 已 running 但 complete/写回未终结。无 snapshot 的 context job 与其余 orphan 使用同一跨实例 grace；超时后可安全接管或确定性终结为 failed/unavailable，不重跑第二 AI。不得因 replayed job 抛错、或因看到既有 attempt 直接 return而永久卡住。进程启动、ACK 丢失与并发通知最终均须 terminal，且同 consumer 最多一个 committed attempt。scanner 已将 context job 终结后，原进程的 late snapshot writeback 必须通过 job-status CAS 失败并整体回滚，不能把 terminal job 改回 succeeded。
