...
9. 对明确需要独立审查的任务，执行 Agent 不得代替独立审查 Agent 宣布任务通过。
10. 未完成适用验证的任务不得标记为 `DONE`。文档整理、格式修复、简单配置和局部低风险修改可由总控 Agent 自检关闭；涉及核心架构、关键业务规则、权限、安全、状态机、核心数据模型、跨模块契约、大规模合并或 MVP 发布时，必须由独立审查或验收 Agent 给出结论。
