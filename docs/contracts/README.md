...
- `memory-persistence-v1.schema.json`、`memory-persistence-v1.md` 与 fixtures：`MEMORY-T5-T8-P2-B-PERSISTENCE-CONTRACT-001` 的 `REVIEW / NOT ACCEPTED / CONTRACT ONLY` 候选。它冻结 checkpoint、layer identity/revision/member、Long projection/job、retention/FK 与 Claim→Evidence authority/reference parity；只保存 typed IDs、revision、status、digest、membership，不保存语义正文、prompt、context、transcript 或 provider payload。当前不代表 Prisma/migration/repository/runtime/provider 已实现或已接收。
