...
5. proposal 的全部 source/claim/evidence refs 必须构成 Context 内闭合子图；merge/compress 至少两个 source；`mark_uncertain` 必须产生 `uncertain`。P1 v1.2 方言固定为 `exact -> single`、`range -> range`、`unknown -> unknown`；只有 `disputed` 可使用 `value_kind/value=null + conflict_set` 且至少两个 proposed claims。
