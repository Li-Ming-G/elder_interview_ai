...
| SPEC-ASR-WIRE-PARAM-001 | 待项目负责人编号 | [PR #29](https://github.com/Li-Ming-G/elder_interview_ai/pull/29) / exact head 与 CI 见当前 GitHub checks | REVIEW / PENDING | docs-only 修正候选；请求项目负责人审查 `speaker_diarization=1`/`enable_speaker_context=0` 是否同时实发并签名、`speaker_context_id` 是否严格省略，以及 ADR-033 是否只部分取代旧事实。执行 Agent 不给出 PASS/DONE/merge 结论 |
