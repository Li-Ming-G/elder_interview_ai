...
| DEV-008A4 | REV-047 | [PR #44](https://github.com/Li-Ming-G/elder_interview_ai/pull/44) / 新 exact head CI 待完成；旧 `f1eea3c` / CI `31666579181` SUCCESS 为用户实测发现问题的历史候选 | REVIEW / PENDING | 限域修复 canonical 自动收尾恢复、unknown create 自动幂等 replay、严格同源回顾重投影与本机删除确认；执行 Agent 只整理审查包，不给出 PASS/DONE，不合并；项目负责人负责新 exact-head GitHub 手动复审 |
