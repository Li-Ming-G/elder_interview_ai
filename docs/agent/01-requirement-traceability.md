...
| REQ-003 | `01` §5.7、§6；`03` §8、§12、§15；`04` §4.6A-4.9D；`05` §3.7、§5；`06` §4-10；ADR-006/018/019/025 | 实时 ASR、确定态转录、流级说话人确认、可信角色与人工修正 | DEV-004A/B1/B2/C1/C2、`SPEC-DEV-004C` 与父 DEV-004 已完成内部 MVP 收口 | C1 stable transcript/trusted-role/calibration seam；C2 单段修正、持久 preview/membership、原子 execute、revision/审计与行内 UI；ASR 降级不影响原始录音和安全结束 | PR #19 head `757bf52`、CI `31310993567`、REV-029、merge `83cdfef`；main `004dacc` CI `31311278529`；父任务由 REV-030 按内部 MVP 边界聚合关闭 | DEV-004 内部 MVP DONE；真实供应商、故障区间和离线补转录后置 HARDEN-ASR-001。deletion scope `NOT IMPLEMENTED / NOT VERIFIED`、CON-023 OPEN | DONE |
...
| REQ-005 | `01` §5.7-5.8、§6、§9；`04` §§4.28-4.43；`05` §§3.9-3.10；`07` §§3-5/§9/§12-15；ADR-026/027 | 后台 current memory 改善当前下一问与第二次开场；自动记忆直接使用但保留 evidence/version；冲突澄清、明确更正、快照/未来资格/visibility、会后实际问题与完整过程记录 | DEV-004/C1/C2、DISC-006 DONE；`SPEC-DEV-006` REVIEW；`DEV-006` BLOCKED | C 生产 trusted role/revision/membership；SPEC 定向候选冻结逐业务输出 derived 一对一、整版 actual catalog、三类 dependency manifest、三类 retention root、QuestionEvidenceModule、两阶段并发、删除与 legacy 默认；DEV-006 待 PASS 后实现 | `09` §4、§6.1、§7.4-7.5、§8.2、场景 B；旧 head `2b6a5da`/CI `31321844664` 被 REQUEST_CHANGES，新 exact head/CI 待绑定 | CON-024 RESOLVED；CON-018/023 继续 OPEN。项目负责人 P1=3 定向修复中；普通修正保留快照，硬边界撤下正文；SPEC PASS 前 DEV-006 不得开工 | REVIEW |
| REQ-006 | `01` §6、§9；`03` §9.3；`07` §6-13；ADR-020/024/027 | 单个当前最佳追问、推荐原因、继续倾听、“没用，换一个”、只撤销最近一次更换、边界与降级 | `SPEC-AI-QUESTION-001` BLOCKED、`DEV-007A`（未实现）；R3 只留状态容器 | `05` §3.9、§5.10；replace/undo 幂等、竞态和排除集合恢复待冻结；共享 question history/derived/retention seam 由 SPEC-DEV-006 提供 | `09` §7、场景 A/B/C | 前置固定为 `SPEC-FE-001 DONE + SPEC-DEV-006 项目负责人 PASS/merge`；PR #20 完成治理收口前不得 READY；旧采用/已问/忽略动作继续冻结 | BLOCKED |
...
| REQ-012 | `00` §3、§9；`09` §1.1/§7.3-7.4；`10` §1.1；ADR-013/020/021/026 | 探索期以虚构数据分段建立纵向链路；任务 DONE、内部可用与真实试点分层门禁 | `MVP-V01`、`DEV-002/003/004/005/006/007` | 复用已冻结 REST/事件契约；测试转录和预创建项目仅内部夹具；补转录后置 HARDEN-ASR-001；首个记忆试用以过程记录和定性复盘为主 | DEV-004/005 内部链路已通过；DISC-006 DONE；真实试点仍执行 `09` §13 | 当前推进 SPEC-DEV-006 → DEV-006 → 单问题追问；生产可靠性和质量百分比门禁不抢占首个内部价值验证 | 执行中 |
