...
| REQ-003 | `01` §5.7、§6；`03` §8、§12、§15；`04` §4.6A-4.9D；`05` §3.7、§5；`06` §4-10；ADR-006/018/019/025 | 实时 ASR、确定态转录、流级说话人确认、可信角色与人工修正 | DEV-004A/B1/B2 已合并；`SPEC-DEV-004C` REVIEW；`DEV-004C1/C2` BLOCKED | B1/B2 既有 `/ws/interviews`；候选新增 `speaker_stream_id`、校准快照/动作、单段与持久预览批量修正、revision/membership seam；无公开文字/provider payload 注入 | 既有 B2 final head `73a07cb`、CI `31143035668` PASS；C1/C2 按 `09` 新校准、流隔离、消费门禁、原子修正和 Android 矩阵待执行 | REV-012/013/014 PASS；DISC-004C 已定稿；SPEC 候选等待项目负责人 GitHub 审查，CON-014 保持 OPEN | IN_PROGRESS |
...
| REQ-005 | `01` §5.7、§6、§9；`07` §3-5 | 项目级结构化记忆、证据回链与冲突处理；只消费可信 elder/interviewer，排除 unknown 与校准控制内容 | `DEV-006`（未实现）；依赖 `SPEC-DEV-004C` 与 `DEV-004C1` PASS | `05` §3.7、§3.10；消费 `speaker_role_revision` 与受影响 segment membership，重算仍由 DEV-006 负责 | `09` §4、§6.1、§7、场景 B | 待执行；不因 DEV-004C 产生失效 seam 而宣称 AI 重算已实现 | 规划 |
