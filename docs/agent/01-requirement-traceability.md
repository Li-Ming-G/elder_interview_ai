...
|---|---|---|---|---|---|---|---|
| REQ-001 | `01` §6、§9；`03` §4-6；`08` §4-5 | 项目、服务条款、授权与访问门禁 | `apps/api/src/project-foundation` 仅合同中立策略；正式持久化/API 未实现 | `05` §3.1-3.5 | 22 个领域/访问单测；正式集成待 CON-009 | 创建者 owner 越权 P1 已修复；CON-009 OPEN | 部分实现/阻塞 |
