...
| REQ-004 | `01` §8；`03` §7-12、§17.2/§18；`04` §4.6/§4.25-4.26；`05` §3.5；`06` §9-11；ADR-020/021/022 | 首次访谈准备页、转录优先工作台、链路状态和安全结束；项目管理/完整回顾后置 | `DEV-005A/B` 已实现；`DEV-005C` 四项 P1 定向修复候选完成，`DEV-005D` 待实现 | stop/recover、统一资源锁、冻结 commitments、ASR ending seam、持久重启事实与稳定幂等响应 | `09` 场景 A/C、§10.1；真实 PostgreSQL barrier、ASR success/unavailable/timeout/restart、completed/failed/idempotency 回归 | REV-019 对旧 head REQUEST_CHANGES；新候选本地完整门禁通过，等待 GitHub CI 与定向复审；DEV-005D 继续等待 C PASS | 执行中 |
...
| REQ-012 | `00` §3、§9；`09` §1.1；`10` §1.1；ADR-013/020/021 | 探索期以虚构数据分段建立纵向链路，当前优先首次访谈三页闭环；任务 DONE 与真实试点分层门禁 | `MVP-V01`、`DEV-002/003/004/005/006/007` | 复用已冻结 REST/事件契约；测试转录和预创建项目仅内部夹具 | 首次访谈 Chromium 纵向 E2E；真实试点仍执行 `09` §13 | DEV-005A/B 已 PASS/DONE；完整闭环仍受 DEV-005C 定向修复与 DEV-005D 约束 | 执行中 |
