...
- 受影响任务：`DEV-001B` 最终安全验收；不阻塞密码、会话、限流、Origin/CSRF、CLI 和已知用户路径的继续实现。
- 临时处理：未知账号与错误密码继续统一返回 `INVALID_CREDENTIALS`，使用不含邮箱/IP 原文的持久化限流摘要保护；不得把限流记录或普通技术日志冒充正式审计完成。任务保持 `IN_PROGRESS`，不得在本冲突解决前宣称审计验收完整。
