...
- T0 语义：记录每次 question-orchestration decision/generation attempt，包括 Director 未调用的 continue、unavailable、门禁拒绝、timeout、retry/stale fence；每个用户可见结果恰好一条不可变、受 retention 管理的 trace。
...
- Decision Trace v1 不改字节；v1.1 新增 `trace_kind`、`memory_outcome`、typed source roots、layer/resolution membership refs 和 strict loader/version/raw schema digest。memory trace 不得伪造 question Director 事实；由于现有 question-only columns 非 NULL，P2-C 按 compatibility artifact 使用 `director_invoked=false`、`context_revision=0`、`stage_timings_json={}` 等明确 neutral sentinel，并由 memory reader 校验后忽略。
...
- 稳定 identity：新增可被真实 FK 引用的 `memory_resolution_authority` registry；现有 `MemoryResolution.id` 继续是 append-only revision-row ID，物理 `authority_id` FK 为 legacy-compatible nullable，P2 新写入必须由 CHECK/trigger/reader gate 非空并指向 registry；legacy NULL 保持 unavailable，不得被 P2 reader/checkpoint 选中。同一 semantic slot 的 revision chain 共享 `authority_id`，并以 `(authority_id, resolution_revision)` 唯一。A1/P2-B 的 `resolution_id` 在 P2-C durable mapping 中指该稳定 authority identity，而非某一 revision row。`origin_thread_id` 必填，Long 沿用 source Mid identity thread；具体 compatibility 见 `docs/contracts/memory-persistence-p2c-compatibility-v1.md`。
...
- Contract compatibility：durable checkpoint分开真实P1 terminal ref与P2 producer job；online/final严格使用不同必填矩阵。现有AiJob Int policy/retention字段不改型且只作为legacy snapshot；P2-B string `policy_revision`/`retention_policy_version` 仅来自一对一 job projection 的 `p2_policy_revision`/`p2_retention_policy_version` VARCHAR，禁止隐式 cast。pending/running及所有非success job不预留最终target revision/new identity ID。
- Retention与版本：自动P2对象继承现有AiJob root，typed target child 只允许 `checkpoint|layer_revision|job|trace`，不新增 `long_projection` root；所有 live refs 使用 RESTRICT，唯一 SET NULL 为 cleanup/audit pointer，不使用数据库 CASCADE。`MemoryRetentionRoot` ownership或`RetentionState`枚举不扩展；P2-B expanded states映射到到期条件/cleanup阶段/audit。历史P1 v1.1 snapshot只读，当前新producer只允许v1.2，checkpoint按版本分派验证。
- 兼容性修复：独立 `MemoryEvidenceAuthority` 承担跨 claim evidence identity；P2 freeze/job/checkpoint/Trace source refs 共享 `deletion_scope_digest`；memory Trace 的现有非空 question-only 字段使用明确 neutral sentinel；P2 error codes 与 physical FK manifest 由 compatibility artifact 冻结。该修复仍是 `Proposed / CONTRACT-FIRST`，不代表 runtime PASS。
- physical manifest：`docs/contracts/memory-persistence-p2c-physical-fk-v1.json` 逐条冻结 authority/checkpoint/identity/revision/member/Long/source/evidence/Trace/retention/cleanup FK 的 name/from/to/nullable/onDelete/deferrable，并固定 P2-B view boundary、retention root view 列映射、26 条 predecessor migration ID/SHA-256、fingerprint、advisory lock、cursor、mode 和 transaction/retry 规则。旧 cleanup request ID 不得冒充 `cleanup_job_id`，P2-B `source_revision` 固定映射 evidence `authority_revision`。
- DB/runtime 定向 correction：`deletion_scope_digest` 是 non-null SHA-256/CAS scalar，不是 FK；Trace source reference 必须使用 typed nullable source columns、physical FKs 与 source-kind/ exactly-one CHECK；`20260814210000_dev008b2_review_invariants` 的实际 SHA-256 由当前主工作树和 `git show 0ea5f47` 重算为 `92a8faae0197f0390c3547664872a4a0ad167f9a139daea6c44c1393e31d049e`；26 条 predecessor migration 的 canonical-json-v1 fingerprint 固定为 `2b1a4ba4a0a20f2e986cec7de2c9863dd7a67673abb033406374517e4bafcea6`，manifest 必须自校验 `migrations.length === 26` 与 upgrade expected count=26。
- typed source nullable correction：`decision_trace_memory_source_reference` 的 checkpoint/job/input-segment/evidence/resolution 五个 typed FK 列全部物理 nullable；source-kind CHECK 与 `num_nonnulls(...)=1` 决定唯一有效 target。`trace_id/source_kind/source_revision/membership_digest/deletion_scope_digest/input_order` 等 row authority 列继续 NOT NULL；不得因 union nullable 放宽真正必填列。
- pre-integration REQUEST_CHANGES correction：dirty snapshot `c3eaa4ae…` 的正式结论为 `P0=0/P1=3/P2=0`。Trace source-kind 增加五值闭域；`decision_trace` parent 独占 trace/outcome/decision/status/stage/error/sentinel 与真实 lifecycle，`decision_trace_memory_semantic` child 只持 job/source/commit digest；`memory_evidence_authority.authority_revision` 成为唯一 evidence revision owner，claim evidence/bridge 只做 pair parity；P2 string policy source 只允许 `memory_p2_job_projection`，checkpoint 仅保留 legacy Int snapshot 和独立 contract identity。该窄修是待复审候选，不改变 ADR 状态、不构成 PASS/DONE。
