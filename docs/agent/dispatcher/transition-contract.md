...
- Queue-wide READY selection is authoritative after reconciliation: exactly one eligible READY task transitions to `IN_PROGRESS`, zero yields `NO_READY_TASK`, and more than one yields `DISPATCHER_STATE_INVALID`.
