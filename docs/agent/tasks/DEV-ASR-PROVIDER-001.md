...
- 本轮禁止新增腾讯连接或费用；provider/replay/desktop/Android 证据仍只绑定旧 head `af99d91`。actual billing/SKU unknown、CON-027 OPEN、真实长者/PII 禁止，均不变。

## 最新 main closeout（2026-08-14）

- 已逐路径把历史候选语义迁移到最新 main，同时保留 1 MiB stop manifest body limit、runtime override 和 producer/access 新事实；没有 merge 旧分支或覆盖旧工作树。
- 修复最新 main 的遗留数据库索引：追加 migration 仅删除已被 stream-scoped 唯一索引替代的 `speaker_mapping_current_key`，允许重连新 speaker stream 合法复用供应商 label；旧 migration 历史不改写，并新增回归测试。
- 新 head 本地门禁：unit 62 files / 395 tests；fresh PostgreSQL integration 14 files / 89 tests、auth 4 files / 23 tests；普通 Chromium 27/27、auth Chromium 5/5；15 migrations deploy/status；正式 Chromium R4 300000ms 通过（296 chunks、单一 audio object、两代 stream、双人确认、pending=0、manifest complete、sticky degraded、session completed）。
- 腾讯真实 provider/replay/桌面/Android 证据仍严格绑定历史 head `af99d91`，不冒充当前 closeout head 的新 provider 调用；当前代码语义由离线、集成与正式浏览器门禁复核。实际 SKU/账单仍未知，CON-027 仍 OPEN。
- 首轮集成 race 夹具在 adapter 释放过早时失败，已改为先证明 stop 在 adapter 阻塞期间完成；复跑通过。复用数据库曾因固定题库版本残留失败，fresh DB 全绿。R4 还永久记录错误项目名、错误数据库角色、缺会话/正式 consent policy/新校准门禁等环境与夹具失败，不删除历史。
...
契约扩张型 Prisma migration、第二 provider/diarizer、gap/backfill、真实 LLM、真实长者/PII、生产部署。closeout 追加的索引清理 migration 只移除与既有 stream-scoped 正式事实冲突的遗留索引，不扩张模型；若需要改变公共状态、数据库权威事实、校准体验或数据授权，先暂停并更新 SPEC。
