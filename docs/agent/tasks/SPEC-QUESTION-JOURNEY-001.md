...
3. 最小 13 字段、v1 条件码、全量校验、不可变 bank version、draft/active/retired 和原子激活。
4. `project_original|verified|unverified|fixture_only` 的许可门禁；公开可访问不等于可复制/改写/导入。
5. `verbatim|lightly_adapted` 的允许/禁止边界，以及题库原题与 transcript/memory 的双重 provenance。
...
- 恰好一次独立只读复核，主模式 `Correction mode`；
- 已吸收三项高影响修正：原子内容发布语义、可验收阶段判定、轻调双重 provenance；
- 复核不替代项目负责人最终审查。
