...
1. 正常录制 1440×900 以顶部/转录/建议约 8%/79%/13% 为视觉基准，390×844 约 9%/73%/18%，允许约 ±3 个百分点；320×568 顶部不高于 72px、活动建议不高于 120px、转录不低于可用高度 60%。实现用受控 header/footer 与中间 `1fr`，不硬编码百分比。
2. 正常态页面本身不纵向滚动；转录是最大连续区域和唯一主要纵向滚动区。“继续倾听”或建议不可用时建议区收缩到约 64–72px。
3. 顶部只常驻长者、时长、采集+本浏览器 archive 的一句安全摘要和结束；转录状态位于转录标题旁；“保存状态”按来源展开五类事实与最后核验时间。关键异常提升但不抢焦点。
4. `interrupted|stopping|processing|completed|failed|NO_AUDIO_CAPTURED` 按当前用户任务重新分配重心，不强行保持转录 80%；只有结束确认使用真正 modal。
5. 所有设备使用左侧 52–64px（手机）元数据栏与右侧正文的高密度转录，不使用聊天气泡。确定态不重复标签，只有中间态显示“识别中”。
6. 手机是完整访谈主设备；首轮正式支持 Android Chrome，iPhone Safari 延期。屏幕旋转只重排，不刷新、不重新申请麦克风或创建 capture；后台、锁屏、可见性和设备中断由 R2 基于真机证据冻结为继续可靠采集或显式 interrupted，R3 不得猜测。
7. 视口验收至少覆盖 1440×900、1024×768、768×1024、390×844、320×568，并由 R4 增加至少一台 Android Chrome 真机 5–10 分钟完整访谈证据。
