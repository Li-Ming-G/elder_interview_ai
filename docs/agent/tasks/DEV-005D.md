...
- 前置依赖：DEV-005A、DEV-005B、DEV-005C PASS、DISC-005D 候选决定包通过总控验收
...
DEV-005D 暂不下发实现。DISC-005D 必须先明确 `stopping|processing` 的停留/离开规则、录音与转录的通俗文案、`failed` 下一步，以及“查看本次记录”“完成并离开”的真实目标。讨论窗口只产生候选决定包；总控验收并写回正式依据后，本任务才可恢复 `READY`。
