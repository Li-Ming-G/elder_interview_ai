...
SPEC-STAGING-DEPLOY-001 的 Quick/Named 双环境、synthetic/fictional-only 数据分类、唯一 `data_mode=synthetic_only`、同源 HTTPS Web/API/WS、Access/app 双身份、trusted proxy/IP、Windows 无人值守、备份恢复、监控回滚与真实数据新任务门禁见 [`SPEC-STAGING-DEPLOY-001.md`](SPEC-STAGING-DEPLOY-001.md)。old `195c4be2` / CI `31798730203` 的项目负责人 REQUEST_CHANGES（P1=1）永久保留，当前只做该 P1 定向修复并保持 REVIEW；未安装 Cloudflare、未请求 token、未部署。后续 [`DEV-STAGING-DEPLOY-001`](../tasks/DEV-STAGING-DEPLOY-001.md) 在定向复审 PASS/merge 前保持 BLOCKED，且无权实现 real/true。

SEC-AUTH-PUBLIC-001 的匿名登录失败审计、staging/production 安全 Cookie、会话撤销竞态、Access/应用账号分责、默认直接对端 IP 与部署 adapter 门禁见 [`SEC-AUTH-PUBLIC-001.md`](SEC-AUTH-PUBLIC-001.md) 和 [PR #55](https://github.com/Li-Ming-G/elder_interview_ai/pull/55)。原 content head `0101837` / CI `31798421917` 已获项目负责人 PASS（P0/P1/P2=0）；branch-local ADR-041 在 latest-main integration 机械映射为 canonical ADR-042。当前仍为 integration REVIEW，CON-008/DEV-001B REVIEW、公网/真实凭据禁令与 DEV-STAGING-DEPLOY-001 才能实现 trusted ingress/proxy 的边界均未改变。
