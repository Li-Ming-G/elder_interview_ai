...
DEV-LLM-PROVIDER-001A 的 provider-neutral readiness 交接见 [`DEV-LLM-PROVIDER-001A.md`](DEV-LLM-PROVIDER-001A.md) 与 [`REV-056`](../reviews/REV-056.md)。本任务从 `8eb34f9` exact base 开始，父 DEV-LLM-PROVIDER-001 继续 BLOCKED；不安装 SDK/provider、不选择厂商/地区、不建立 active binding、不读取 secret、不调用网络/provider、不加载 v2-draft。独立 reviewer 通道无输出，已按 iteration-coach fallback 完成一次单 Agent Clear 审查，不记作独立结论。
