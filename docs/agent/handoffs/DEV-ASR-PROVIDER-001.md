...
- Offline gates：format/lint/typecheck/build；unit46 files/313；独立PG 13 migrations current、integration13 files/80、auth4 files/23；smoke；Chromium10/10；auth Chromium4/4；R4 formal route1/1。首次auth Chromium端口误配和首次R4旧断言/FK清理失败均保留为环境/测试契约修正记录，正确隔离环境复跑通过。
