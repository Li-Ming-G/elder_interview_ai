...
| DEV-006 | 后台结构化长期记忆最小纵向链路 | 待分配 | READY | DISC-006 DONE、DEV-004C1/C2 DONE、SPEC-DEV-006 PASS/merge | [任务卡](tasks/DEV-006.md)、[执行提示词](prompts/DEV-006.md)、ADR-026/027、`04/05/07/08/09/10` | 实现 shared QuestionEvidenceModule/actual catalog 与后台 current memory；DEV-007 只消费 seam。CON-023 继续 NOT IMPLEMENTED/NOT VERIFIED，不得用 no-op guard 冒充删除覆盖 |
