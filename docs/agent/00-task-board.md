...
| DEV-004 | 实时 ASR 与说话人映射（父任务） | 总控 Agent | IN_PROGRESS | DEV-003 | [父任务卡](tasks/DEV-004.md)、ADR-018/019 | DEV-004A 已通过，B1 已 READY、B2 待共享契约提交；父任务不提前关闭 |
...
| DEV-004B1 | 业务 WebSocket 服务端协议核心 | 后端实时转录实现 Agent（`dev004b1_backend_impl`） | IN_PROGRESS | DEV-004A、ADR-019 | [任务卡](tasks/DEV-004B1.md)、`05` §5、`06` §4/§9/§11、`146bfde` | 静态 WS、JSON/base64 合成 PCM、单进程短时恢复；不含 Web UI、真实 ASR 或 AudioWorklet |
