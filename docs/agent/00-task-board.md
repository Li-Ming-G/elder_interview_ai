...
| DEV-ASR-PROVIDER-001 | 腾讯实时 ASR V2 adapter v2 实现与真实验收 | 待分配 | READY | SPEC-ASR-PROVIDER-001 项目负责人 exact-head PASS/merge | [任务卡](tasks/DEV-ASR-PROVIDER-001.md)、`09` §15、v2 machine contract | 仅允许腾讯单 provider、完全虚构测试数据和安全后端测试 secret；不含第二 provider、gap/backfill、Prisma migration、真实 LLM、真实长者或部署 |
