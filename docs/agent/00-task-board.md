...
| DEV-003 | 原始录音与分片上传（父任务） | 总控 Agent | IN_PROGRESS | DEV-003A、DEV-003B、DEV-003C | `06`、`09`、REV-010 | DEV-003A/B 内部原型已通过；DEV-003C 正在接通可靠自动上传并处理两项存储 P2 |
...
| DEV-003C | 浏览器可靠上传编排与存储恢复加固 | 总控 Agent（协调前后端实现） | IN_PROGRESS | DEV-003A、DEV-003B、REV-010 | [任务卡](tasks/DEV-003C.md)、ADR-017 | 仅限虚构音频内部链路；完成后 push GitHub 并等待项目负责人审查 |
