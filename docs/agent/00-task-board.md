...
| DEV-004 | 实时 ASR 与说话人映射（父任务） | 总控 Agent | IN_PROGRESS | DEV-003 | [父任务卡](tasks/DEV-004.md)、ADR-018 | DEV-004A 已启动；父任务不因 A 完成而关闭 |
| DEV-004A | 确定态转录证据核心与供应商中立适配器 | 后端转录实现 Agent（`dev004a_backend_impl`） | REVIEW | DEV-003、ADR-018、CON-015/016 | [任务卡](tasks/DEV-004A.md)、`04` 至 `06`、`b34205f`、CI `30886820301` PASS | GitHub 全门禁通过；等待项目负责人按 PR #3 最终 head 审查 |
