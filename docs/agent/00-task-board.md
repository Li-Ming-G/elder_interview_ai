...
| `PFC-04-SUGGESTION-RECOVERY` | `IN_PROGRESS` | PR #130 current-head Architect `REQUEST_CHANGES`: disable/unavailable manual next when `current === null`, preserve usable retry, and prove retry success restores manual next | [`tasks/PFC-04-SUGGESTION-RECOVERY.md`](tasks/PFC-04-SUGGESTION-RECOVERY.md) | `luna-high` | `130` | `PFC-05-ROUTE-ACTION-CLOSURE` |
...
`PFC-01-NEW-INTENT-TRUTH` through `PFC-03-RECORDING-NAV-SAFETY` are `DONE`. `PFC-04-SUGGESTION-RECOVERY` is the sole active `IN_PROGRESS` task on PR #130 for bounded Architect-requested repair.
