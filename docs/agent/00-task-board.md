...
| SPEC-ASR-WIRE-PARAM-001 | 腾讯 V2 话者分离 wire 参数正式修正 | 腾讯 wire 契约修正 Agent | REVIEW | SPEC-ASR-PROVIDER-001 DONE；官方一手证据纠正 ADR-032 的窄供应商事实 | [任务卡](tasks/SPEC-ASR-WIRE-PARAM-001.md)、[交接](handoffs/SPEC-ASR-WIRE-PARAM-001.md)、[PR #29](https://github.com/Li-Ming-G/elder_interview_ai/pull/29)、ADR-033、正式腾讯 profile | docs-only；等待项目负责人手动 GitHub 审查 non-Draft PR exact head，不自宣 PASS/DONE/merge。获 PASS 后才允许 DEV 用同一虚构 TTS PCM、`reconnect=0` 做一次受控诊断；不证明 close 1005 根因 |
| DEV-ASR-PROVIDER-001 | 腾讯实时 ASR V2 adapter v2 实现与真实验收 | 待分配 | READY | SPEC-ASR-PROVIDER-001 项目负责人 exact-head PASS/merge；受控 wire 诊断另等 SPEC-ASR-WIRE-PARAM-001 PASS | [任务卡](tasks/DEV-ASR-PROVIDER-001.md)、`09` §15、v2 machine contract | 仅允许腾讯单 provider、完全虚构测试数据和安全后端测试 secret；一次 `reconnect=0` 诊断须等待新 wire 修正人工 PASS；不含第二 provider、gap/backfill、Prisma migration、真实 LLM、真实长者或部署 |
