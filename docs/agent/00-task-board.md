...
| `PFC-03-RECORDING-NAV-SAFETY` | `BLOCKED` | `MAIN_VERIFY_FAILED`: PR #129 merged as `ee01cf0c42d7041bc7d4524c21a874eb51245787`; exact-current-main CI run `33533271266` failed in realtime auth E2E connection; mechanically retry recovery on later pulse | [`tasks/PFC-03-RECORDING-NAV-SAFETY.md`](tasks/PFC-03-RECORDING-NAV-SAFETY.md) | `luna-high` | `129` | `PFC-04-SUGGESTION-RECOVERY` |
...
`PFC-01-NEW-INTENT-TRUTH` and `PFC-02-PRESTART-DISCARD` are `DONE`. PR #129 is merged, but `PFC-03-RECORDING-NAV-SAFETY` is the sole active `BLOCKED / MAIN_VERIFY_FAILED` task until applicable exact-current-main CI succeeds.
...
No task remains `READY` while `PFC-03-RECORDING-NAV-SAFETY` awaits mechanical main-verification recovery. `PFC-04` through `PFC-07` remain `DEFERRED` until mechanically unlocked by predecessor DONE.
