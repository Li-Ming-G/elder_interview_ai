...
| SPEC-ASR-WIRE-PARAM-001 | 腾讯 V2 话者分离 wire 参数正式修正 | 腾讯 wire 契约修正 Agent | REVIEW | SPEC-ASR-PROVIDER-001 DONE；官方一手证据纠正 ADR-032 的窄供应商事实 | [任务卡](tasks/SPEC-ASR-WIRE-PARAM-001.md)、[交接](handoffs/SPEC-ASR-WIRE-PARAM-001.md)、ADR-033、正式腾讯 profile | docs-only；等待项目负责人手动 GitHub 审查 non-Draft PR exact head，不自宣 PASS/DONE/merge。获 PASS 后才允许 DEV 用同一虚构 TTS PCM、`reconnect=0` 做一次受控诊断；不证明 close 1005 根因 |
