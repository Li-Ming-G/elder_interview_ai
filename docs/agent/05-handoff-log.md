...
| DEV-008B1 | [候选交接](handoffs/DEV-008B1.md)、[任务卡](tasks/DEV-008B1.md)、[PR #51](https://github.com/Li-Ming-G/elder_interview_ai/pull/51)、REV-050 | 项目负责人对新 exact head 定向复审 | REVIEW；old exact head `efac390d` / CI `31769522565` 获正式 REQUEST_CHANGES（P0=0/P1=3/P2=0），历史永久保留。旧 endpoint 首场限定、repeat 当前页 mic/device、server-pointer-only unknown 恢复已定向修复并完成本地全门禁；等待新 exact-head CI。默认/生产 policy unavailable；SPEC-CONSENT-TEXT-POLICY-001 仍 BLOCKED，B2 未实现，不得 PASS/DONE/merge |
