...
|---|---|---|---|
| SPEC-STAGING-DEPLOY-001 | [PR #54](https://github.com/Li-Ming-G/elder_interview_ai/pull/54)、[任务卡](tasks/SPEC-STAGING-DEPLOY-001.md)、[交接](handoffs/SPEC-STAGING-DEPLOY-001.md)、[正式候选](../contracts/staging-deployment-v1.md)、ADR-041、REQ-020 | 项目负责人 exact-head 审查；通过后才交 DEV-STAGING-DEPLOY-001 | REVIEW；首个 `235a3df6` / CI `31798290760` SUCCESS，补证后最终 head 仍须自己的完整 CI。唯一 iteration-coach Correction 已吸收；只完成 docs/contract，未安装 Cloudflare、未请求 token、未部署、未处理真实数据。DEV 保持 BLOCKED，`real_data_allowed=false` |
| DEV-STAGING-DEPLOY-001 | [任务卡](tasks/DEV-STAGING-DEPLOY-001.md)、staging contract §12、`09` §20 | 后续独立实现任务 | BLOCKED；等待 SPEC exact-head PASS/merge。只允许 Named synthetic staging；真实数据、HA、云迁移与既有身份/授权/删除/provider/QA 门禁不在解锁范围 |
