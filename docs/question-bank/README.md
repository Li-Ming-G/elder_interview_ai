...
模板固定为 14 列，`purpose` 必填并复用正式 11 值。多项 `applicable_when` 是 all-of/AND，多项 `inapplicable_when` 是 any-of/OR，排除优先；整体空值允许，未知、空 token、重复或同码跨字段冲突整批拒绝。fixture 与正式文件必须走同一 parser/validator。
