...
const MIGRATION_NAME = '20260822120000_memory_p2_c_database';
...
describe('MEMORY-T5-T8-P2-C-RUNTIME-001 migration', () => {
  ...
  it('pins authority/revision, member/order and predecessor uniqueness constraints', async () => {
    const indexes = await client.query<{ indexname: string; indexdef: string }>(
      ...
      [
        [
          'memory_resolution_authority_id_resolution_revision_key',
          'memory_checkpoint_member_authority_key',
          'memory_checkpoint_member_order_key',
          'memory_layer_revision_number_key',
          'memory_layer_revision_predecessor_key',
          'memory_layer_revision_member_claim_key',
          'memory_layer_revision_member_order_key',
        ],
      ],
    );
    expect(indexes.rows.map((row) => row.indexname)).toEqual([
    ...
    const predecessor = await client.query<{ definition: string }>(
      `SELECT pg_get_constraintdef(oid) AS definition
         FROM pg_constraint
        WHERE conname = 'memory_layer_revision_predecessor_shape_ck'`,
    );
    expect(predecessor.rows[0]?.definition).toContain('revision_no');
    expect(predecessor.rows[0]?.definition).toContain('predecessor_revision_id');
  });
