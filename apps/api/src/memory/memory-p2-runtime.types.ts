import type { DecisionTraceStatus } from '../ai-runtime/decision-trace.service.js';
...
/** Stable P2-C compatibility registry; free-form errors must not cross this seam. */
...
export type MemoryP2ProgressEvent =
  | { jobId: string; sourceManifestHash: string; stage: 'context_validated' }
  | { jobId: string; sourceManifestHash: string; stage: 'proposal_received' }
  | {
      jobId: string;
  ...
  | { jobId: string; sourceManifestHash: string; stage: 'authority_checked' };

/** Progress is reference-only; payloads and CAS tokens never enter this port. */
...
export type MemoryP2RunResult =
  ...
  | { jobId: string; outcome: 'in_progress'; status: 'pending' | 'running' }
  | {
      errorCode: MemoryP2ErrorCode;
      jobId: string;
      outcome: 'terminal';
  ...
  | { errorCode: MemoryP2ErrorCode; jobId: string; outcome: 'rebase_required' }
  | { errorCode: MemoryP2ErrorCode; outcome: 'not_frozen'; requestIdentity: string }
  | {
      errorCode: MemoryP2ErrorCode;
      jobId: string;
